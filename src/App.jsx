    ...
    <div className="showcase-page">
      ...
      <div className="showcase-layout">
        <main className="showcase-main">
          ...
          {type === 'brand' && toolbarTab === 1 ? <section className="product-library-panel glass-card" aria-label="品牌产品库"><h2>品牌成品与设计方案</h2>{brandImage ? <article><img src={brandImage} alt="已自动保存的品牌成品" /><div><strong>已自动保存到品牌创作 / 产品库</strong><small>可继续调整、生成变体或下载</small></div></article> : <div className="empty-state"><Sparkles size={22} /><strong>还没有品牌成品</strong><span>在“素材创作”中选择品牌资产并生成第一个方案。</span></div>}</section> : <section className={`case-grid case-grid--${type}`} aria-label={`${config.title}案例`}>
        ...
        <aside className="case-inspector glass-card">
